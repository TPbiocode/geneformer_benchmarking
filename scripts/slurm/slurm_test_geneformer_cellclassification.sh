#!/bin/bash
#!/bin/bash
#SBATCH --job-name=geneformer_test         # Job name (can override at submit time with --job-name)
...
CONFIG_FILE_BASE="${BASE_DIR}/scripts/slurm/config_test_myexp.sh"
CONFIG_FILE_WORKSPACE="${GENEFORMER_WORKSPACE}/scripts/slurm/config_test_myexp.sh"
...
# ---------------------------------------------
SCRATCH_LABEL="${OUTPUT_PREFIX:-v1_10m_l3_test_eval}"
SCRATCH_DIR="/scratch/$USER/${SCRATCH_LABEL}_$SLURM_JOB_ID"
mkdir -p $SCRATCH_DIR
cd $SCRATCH_DIR

# Copy the Python script and any required files from project folder
cp ${BASE_DIR}/scripts/slurm/test_metrics_geneformer_cellclassification.py .
...
echo "Geneformer Model: ${MODEL_VERSION:-<python default>}"
which python
python --version
...
python test_metrics_geneformer_cellclassification.py
